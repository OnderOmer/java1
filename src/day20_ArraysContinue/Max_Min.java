    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i <= lastindex; i++  ){
            ...
            if(arr[i] >= max){  // array's index are compared with each other,
                // and whichever is greater will be assigned to the max
                max = arr[i];
            }

            if(arr[i] < min){ // array's index are compared with each other,
                // and whichever is smaller will be assigned to the min
                min = arr[i];
            }
